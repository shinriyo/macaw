...
exports.openRulesView = openRulesView;
...
let panel;
async function openRulesView(context) {
    if (panel) {
        panel.reveal(vscode.ViewColumn.One);
        panel.webview.postMessage({ type: 'state', state: getRulesViewState() });
        return;
    }
    panel = vscode.window.createWebviewPanel('macawRules', 'Macaw Rules', vscode.ViewColumn.One, {
        enableScripts: true,
...
function getRulesViewState() {
    const config = (0, config_1.getConfig)();
    const folder = (0, pathRules_1.getPrimaryWorkspaceFolder)();
    return {
        ...config,
        currentRoot: folder ? (0, pathRules_1.pathToTildePattern)(folder.uri.fsPath) : '',
        currentFolderName: folder ? (0, pathRules_1.getFolderName)(folder) : ''
    };
}
...
async function clearWorkspaceMacawSettings(config) {
...
function getHtml(webview, state) {
    ...
    const encodedLanguages = JSON.stringify(Object.entries(language_1.languageNames).map(([id, name]) => ({
        id,
        name,
        color: language_1.defaultLanguageColors[id]
    }))).replace(/</g, '\\u003c');
    return `<!DOCTYPE html>
<html lang="en">
<head>
...
//# sourceMappingURL=rulesView.js.map